<?php
// translations/packages-data.php

return [
    'women_classic' => [
        'details' => [
            [
                'title' => 'EXAMINATIONS',
                'items' => [
                    'Check Up Examination',
                    'Cardiological Examination',
                    'Gynecological Examination',
                ]
            ],
            [
                'title' => 'RADIOLOGICAL EXAMINATIONS',
                'items' => [
                    'Chest X-Ray Posterior / Anterior',
                    'Thyroid Ultrasonography',
                    'Whole Abdominal Ultrasonography',
                    'Breast Ultrasonography (Two Sides)',
                    'Mammography (Bilateral)',
                ]
            ],
            [
                'title' => 'CARDIOLOGY EXAMINATIONS',
                'items' => [
                    'ECG',
                    'Echocardiography (Doppler+Colored+M+B Mode)',
                ]
            ],
            [
                'title' => 'LABORATORY EXAMINATIONS',
                'items' => [
                    'Alanin Aminotransferase (ALT)',
                    'Aspartate Aminotransferase (AST)',
                    'Iron',
                    'Iron Binding Capacity',
                    'Ferritin',
                    'Magnesium (Mg)',
                    'Folic Acid',
                    'Occult Blood, Stool',
                    'Glucose, Fasting',
                    'Calcium (Ca)',
                    'Cholesterol, HDL',
                    'Cholesterol, LDL (Direct)',
                    'Cholesterol, Total',
                    'Creatinine, Serum',
                    'Check Up Lipid Panel',
                    'Urinalysis',
                    'Triglycerides',
                    'Blood Urea Nitrogen (BUN)',
                    'Uric Acid',
                    'C-Reactive Protein (CRP), Quantitative',
                    'Whole Blood Count (Expanded Panel)',
                    'Insulin',
                    'Thyroid Stimulating Hormone (TSH)',
                    'Thyroxin, Free (FT4)',
                    'Hepatitis Bs Antigen (HBsAg)',
                    'Hepatitis Bs Antibody (anti-HBs)',
                    'Hepatitis C Antibody (anti-HCV)',
                ]
            ],
            [
                'title' => 'PATHOLOGICAL EXAMINATIONS',
                'items' => [
                    'Cytopathology, Thin Prep',
                ]
            ],
        ]
    ],
    'women_gold' => [
        'details' => [
            [
                'title' => 'EXAMINATIONS',
                'items' => [
                    'Check Up examination',
                    'Cardiological Examination',
                    'Gynecological Examination',
                    'Dermatology Examination or Eye Examination',
                    'Dentist Examination',
                    'Diet Examination',
                ]
            ],
            [
                'title' => 'RADIOLOGICAL EXAMINATIONS',
                'items' => [
                    'Chest x-ray Posterior / Anterior',
                    'Thyroid Ultrasonography',
                    'Whole Abdominal Ultrasonography',
                    'Breast Ultrasonography (Two Sides)',
                    'Mammography (Bilateral)',
                    'Bone Density Measurement (Whole Body)',
                    'Panoramic X-Ray',
                ]
            ],
            [
                'title' => 'CARDIOLOGY EXAMINATIONS',
                'items' => [
                    'ECG',
                    'Echocardiography (Doppler + Colored + M + B Mode)',
                ]
            ],
            [
                'title' => 'LABORATORY EXAMINATIONS',
                'items' => [
                    'Alanin Aminotransferase (ALT)',
                    'Aspartate Aminotransferase (AST)',
                    'Iron',
                    'Iron Binding Capacity',
                    'Ferritin',
                    'Magnesium (Mg)',
                    'Folic Acid',
                    'Occult Blood, Stool',
                    'Glucose, Fasting',
                    'Hemoglobin A1c (HbA1c)',
                    'Calcium (Ca)',
                    'Cholesterol, HDL',
                    'Cholesterol, LDL (Direct)',
                    'Cholesterol, Total',
                    'Creatinine, Serum',
                    'Lipase',
                    'Lipoprotein (a) (Lp(a))',
                    'Urinalysis',
                    'Triglycerides',
                    'Blood Urea Nitrogen (BUN)',
                    'Uric Acid',
                    'Vitamin B12',
                    'Vitamin D, 25-Hydroxy',
                    'C-Reactive Protein (CRP), Quantitative',
                    'Whole Blood Count (Expanded Panel)',
                    'Insulin',
                    'Thyroid Stimulating Hormone (TSH)',
                    'Thyroxin, Free (FT4)',
                    'Hepatitis Bs Antigen (HBsAg)',
                    'Hepatitis Bs Antibody (anti-HBs)',
                    'Hepatitis C Antibody (anti-HCV)',
                ]
            ],
            [
                'title' => 'PATHOLOGICAL EXAMINATIONS',
                'items' => [
                    'Cytopathology, Thin Prep',
                ]
            ],
            [
                'title' => 'EVALUATION of BODY DATA',
                'items' => [
                    'Bioimpedancemetry for Determination of Body Composition',
                ]
            ],
        ]
    ],
    'women_executive' => [
        'details' => [
            [
                'title' => 'EXAMINATIONS',
                'items' => [
                    'Check Up Examination',
                    'Ear-Nose-Throat Examination',
                    'Gynecologycal Examination',
                    'Chest Diseases Examination',
                    'Eye Diseases Examination',
                    'Dermatology Examination',
                    'Cardiology Examination',
                    'Diet Examination',
                ]
            ],
            [
                'title' => 'DENTAL SERVICES',
                'items' => [
                    'Dentist Examination',
                    'Panoramic X-Ray',
                    'Upper Jaw Tartat Removel',
                    'Lower Jaw Tartar Removel',
                ]
            ],
            [
                'title' => 'CARDIOLOGY EXAMINATIONS',
                'items' => [
                    'ECG with Effort',
                    'ECG',
                    'Echocardiography (Doppler+Colored+M+B Mode)',
                    'Tension Holter (24 Hours)',
                ]
            ],
            [
                'title' => 'RADIOLOGICAL EXAMINATIONS',
                'items' => [
                    'Cardiac CT - Calcium Scoring',
                    'Thorax CT (for screening purpose / low dose)',
                    'Thyroid Ultrasonography',
                    'Whole Abdominal Ultrasonography',
                    'Breast Ultrasonography (two sides)',
                    'Carotis Bilateral Color Doppler USG',
                    'Mammography (Bilateral)',
                    'Bone Density Measurement (Femur)',
                    'Bone Density Measurement (Lumbar AP)',
                    'MRI, Angiography (Intracranial)(Non-contrast)(3.0 T)',
                    'Brain (Cranium) MR (1,5 T)',
                ]
            ],
            [
                'title' => 'NEUROLOGY EXAMINATIONS',
                'items' => [
                    'Sleep - Polysomnography',
                ]
            ],
            [
                'title' => 'AUDIOLOGY EXAMINATIONS',
                'items' => [
                    'Audio and Timpanometry',
                    'Pure Tone Audiometry',
                ]
            ],
            [
                'title' => 'RESPIRATORY TREATMENT TESTS',
                'items' => [
                    'Respiratory Function Test',
                ]
            ],
            [
                'title' => 'INITIATIVES',
                'items' => [
                    'Dermatoscopy',
                ]
            ],
            [
                'title' => 'EVALUATION OF BODY DATA',
                'items' => [
                    'Bioimpedancemetry for Determination of Body Composition',
                ]
            ],
            [
                'title' => 'PATHOLOGICAL EXAMINATIONS',
                'items' => [
                    'Cytopathology, thin prep',
                    'Stomach, biopsy',
                    'Column, biopsy',
                ]
            ],
            [
                'title' => 'ENDOSCOPIC APPLICATIONS',
                'items' => [
                    'Esophagoscopy, gastroscopy, duodenoscopy + biopsy',
                    'Colonoscopic polypectomy, 1-3 polyps (with cold/hot snare/hot forceps) (Completely excised polyp with a diameter of 5 mm and above)',
                    'Colonoscopy + biopsy',
                ]
            ],
            [
                'title' => 'ANESTHESIA EXAMINATIONS',
                'items' => [
                    'Sedation and Local Anesthesia',
                ]
            ],
            [
                'title' => 'LABORATORY EXAMINATIONS',
                'items' => [
                    'Alpha-Fetoprotein (AFP)',
                    'Alkaline Phosphatase (ALP)',
                    'Alanin Aminotransferase (ALT)',
                    'Aspartate Aminotransferase (AST)',
                    'Amylase',
                    'Zinc (Zn)',
                    'Iron',
                    'Iron Binding Capacity',
                    'Ferritin',
                    'Folic Acid',
                    'Occult Blood, Stool',
                    'Gamma Glutamyl Transferase (GGT)',
                    'Glucose, Fasting',
                    'Hemoglobin A1c (HbA1c)',
                    'Homocysteine',
                    'Calcium (Ca)',
                    'Chloride (Cl)',
                    'Creatinine, Serum',
                    'Lactate Dehydrogenase (LDH)',
                    'Lipase',
                    'Check Up Lipid Panel',
                    'Lipoprotein (a) (Lp(a))',
                    'Magnesium (Mg)',
                    'Potassium (K)',
                    'Protein, Total',
                    'Sodium (Na)',
                    'Urinalysis',
                    'Blood Urea Nitrogen (BUN)',
                    'Uric Acid',
                    'Vitamin B12',
                    'Vitamin D, 25-Hydroxy',
                    'aPTT',
                    'Whole Blood Count (Expanded panel)',
                    'Prolactin (PRL)',
                    'Thyroid Stimulating Hormone (TSH)',
                    'Thyroxin, Free (FT4)',
                    'Triiodothyronine, Free (FT3)',
                    'Culture, Throat',
                    'Parasites, Stool',
                    'Culture, Urine',
                    'Hepatitis B s Antigen (HBsAg)',
                    'Hepatitis B s Antibody (anti-HBs)',
                    'Hepatitis C Antibody (anti-HCV)',
                    'HIV Antibody (1-2) + p24 Antigen',
                    'Insulin',
                ]
            ],
        ]
    ],
    'men_classic' => [
        'details' => [
            [
                'title' => 'EXAMINATIONS',
                'items' => [
                    'Check Up Examination',
                    'Cardiological Examination',
                    'Urology Examination',
                ]
            ],
            [
                'title' => 'RADIOLOGICAL EXAMINATIONS',
                'items' => [
                    'Chest X-Ray Posterior / Anterior',
                    'Thyroid Ultrasonography',
                    'Whole Abdominal Ultrasonography',
                ]
            ],
            [
                'title' => 'CARDIOLOGY EXAMINATIONS',
                'items' => [
                    'ECG',
                    'Echocardiography (Doppler + Colored + M + B Mode)',
                ]
            ],
            [
                'title' => 'LABORATORY EXAMINATIONS',
                'items' => [
                    'Alanin Aminotransferase (ALT)',
                    'Aspartate Aminotransferase (AST)',
                    'Iron',
                    'Iron Binding Capacity',
                    'Ferritin',
                    'Magnesium (Mg)',
                    'Folic Acid',
                    'Occult Blood, Stool',
                    'Glucose, Fasting',
                    'Calcium (Ca)',
                    'Cholesterol, HDL',
                    'Cholesterol, LDL (Direct)',
                    'Cholesterol, Total',
                    'Creatinine, Serum',
                    'Check Up Lipid Panel',
                    'Urinalysis',
                    'Triglycerides',
                    'Blood Urea Nitrogen (BUN)',
                    'Uric Acid',
                    'C-Reactive Protein (CRP), Quantitative',
                    'Whole Blood Count (Expanded Panel)',
                    'Insulin',
                    'Testosterone, Total',
                    'Testosterone, Bioavailable',
                    'Thyroid Stimulating Hormone (TSH)',
                    'Thyroxin, Free (FT4)',
                    'Prostate Specific Antigen (PSA), Total',
                    'Prostate Specific Antigen (PSA), Free',
                    'Hepatitis Bs Antigen (HBsAg)',
                    'Hepatitis Bs Antibody (anti-HBs)',
                    'Hepatitis C Antibody (anti-HCV)',
                ]
            ],
        ]
    ],
    'men_gold' => [
        'details' => [
            [
                'title' => 'EXAMINATIONS',
                'items' => [
                    'Check Up examination',
                    'Cardiological Examination',
                    'Urology Examination',
                    'Chest Diseases Examination or Eye Examination',
                    'Dentist Examination',
                    'Diet Examination',
                ]
            ],
            [
                'title' => 'RADIOLOGICAL EXAMINATIONS',
                'items' => [
                    'Chest x-ray Posterior / Anterior',
                    'Thorax CT (For Screening Purpose / Low Dose)',
                    'Thyroid Ultrasonography',
                    'Whole Abdominal Ultrasonography',
                    'Panoramic X-Ray',
                ]
            ],
            [
                'title' => 'CARDIOLOGY EXAMINATIONS',
                'items' => [
                    'ECG',
                    'Echocardiography (Doppler + Colored + M + B Mode)',
                    'Exercise Stress ECG',
                ]
            ],
            [
                'title' => 'LABORATORY EXAMINATIONS',
                'items' => [
                    'Alanin Aminotransferase (ALT)',
                    'Aspartate Aminotransferase (AST)',
                    'Iron',
                    'Iron Binding Capacity',
                    'Ferritin',
                    'Magnesium (Mg)',
                    'Folic Acid',
                    'Occult Blood, Stool',
                    'Glucose, Fasting',
                    'Hemoglobin A1c (HbA1c)',
                    'Calcium (Ca)',
                    'Cholesterol, HDL',
                    'Cholesterol, LDL (Direct)',
                    'Cholesterol, Total',
                    'Creatinine, Serum',
                    'Lipase',
                    'Lipoprotein (a) (Lp(a))',
                    'Urinalysis',
                    'Triglycerides',
                    'Blood Urea Nitrogen (BUN)',
                    'Uric Acid',
                    'Vitamin B12',
                    'Vitamin D, 25-Hydroxy',
                    'C-Reactive Protein (CRP), Quantitative',
                    'Whole Blood Count (Expanded Panel)',
                    'Insulin',
                    'Testosterone, Total',
                    'Testosterone, Bioavailable',
                    'Thyroid Stimulating Hormone (TSH)',
                    'Thyroxin, Free (FT4)',
                    'Prostate Specific Antigen (PSA), Total',
                    'Prostate Specific Antigen (PSA), Free',
                    'Hepatitis Bs Antigen (HBsAg)',
                    'Hepatitis Bs Antibody (anti-HBs)',
                    'Hepatitis C Antibody (anti-HCV)',
                ]
            ],
            [
                'title' => 'EVALUATION of BODY DATA',
                'items' => [
                    'Bioimpedancemetry for Determination of Body Composition',
                ]
            ],
        ]
    ],
    'men_executive' => [
        'details' => [
            [
                'title' => 'EXAMINATIONS',
                'items' => [
                    'Check Up Examination',
                    'Ear-Nose-Throat Examination',
                    'Urology Examination',
                    'Chest Diseases Examination',
                    'Eye Diseases Examination',
                    'Dermatology Examination',
                    'Cardiology Examination',
                    'Diet Examination',
                ]
            ],
            [
                'title' => 'DENTAL SERVICES',
                'items' => [
                    'Dentist Examination',
                    'Panoramic X-Ray',
                    'Upper Jaw Tartat Removel',
                    'Lower Jaw Tartar Removel',
                ]
            ],
            [
                'title' => 'CARDIOLOGY EXAMINATIONS',
                'items' => [
                    'ECG with Effort',
                    'ECG',
                    'Echocardiography (Doppler+Colored+M+B Mode)',
                    'Tension Holter (24 Hours)',
                ]
            ],
            [
                'title' => 'RADIOLOGICAL EXAMINATIONS',
                'items' => [
                    'Cardiac CT - Calcium Scoring',
                    'Thorax CT (for screening purpose / low dose)',
                    'Thyroid Ultrasonography',
                    'Whole Abdominal Ultrasonography',
                    'Carotis Bilateral Color Doppler USG',
                    'Bone Density Measurement (FeBone Density Measurement (Femur)',
                    'Bone Density Measurement (Lumbar AP)',
                    'MRI, Angiography (Intracranial)(Non-contrast)(3.0 T)',
                    'Brain (Cranium) MR (1,5 T)',
                ]
            ],
            [
                'title' => 'NEUROLOGY EXAMINATIONS',
                'items' => [
                    'Sleep - Polysomnography',
                ]
            ],
            [
                'title' => 'AUDIOLOGY EXAMINATIONS',
                'items' => [
                    'Audio and Timpanometry',
                    'Pure Tone Audiometry',
                ]
            ],
            [
                'title' => 'RESPIRATORY TREATMENT TESTS',
                'items' => [
                    'Respiratory Function Test',
                ]
            ],
            [
                'title' => 'INITIATIVES',
                'items' => [
                    'Dermatoscopy',
                ]
            ],
            [
                'title' => 'EVALUATION OF BODY DATA',
                'items' => [
                    'Bioimpedancemetry for Determination of Body Composition',
                ]
            ],
            [
                'title' => 'PATHOLOGICAL EXAMINATIONS',
                'items' => [
                    'Stomach, biopsy',
                    'Column, biopsy',
                ]
            ],
            [
                'title' => 'ENDOSCOPIC APPLICATIONS',
                'items' => [
                    'Esophagoscopy, gastroscopy, duodenoscopy + biopsy',
                    'Colonoscopic polypectomy, 1-3 polyps (with cold/hot snare/hot forceps) (Completely excised polyp with a diameter of 5 mm and above)',
                    'Colonoscopy + biopsy',
                ]
            ],
            [
                'title' => 'ANESTHESIA EXAMINATIONS',
                'items' => [
                    'Sedation and Local Anesthesia',
                ]
            ],
            [
                'title' => 'LABORATORY EXAMINATIONS',
                'items' => [
                    'Alpha-Fetoprotein (AFP)',
                    'Alkaline Phosphatase (ALP)',
                    'Alanin Aminotransferase (ALT)',
                    'Aspartate Aminotransferase (AST)',
                    'Amylase',
                    'Zinc (Zn)',
                    'Iron',
                    'Iron Binding Capacity',
                    'Ferritin',
                    'Folic Acid',
                    'Occult Blood, Stool',
                    'Gamma Glutamyl Transferase (GGT)',
                    'Glucose, Fasting',
                    'Hemoglobin A1c (HbA1c)',
                    'Homocysteine',
                    'Calcium (Ca)',
                    'Chloride (Cl)',
                    'Creatinine, Serum',
                    'Lactate Dehydrogenase (LDH)',
                    'Lipase',
                    'Check Up Lipid Panel',
                    'Lipoprotein (a) (Lp(a))',
                    'Magnesium (Mg)',
                    'Potassium (K)',
                    'Protein, Total',
                    'Sodium (Na)',
                    'Urinalysis',
                    'Blood Urea Nitrogen (BUN)',
                    'Uric Acid',
                    'Vitamin B12',
                    'Vitamin D, 25-Hydroxy',
                    'aPTT',
                    'Whole Blood Count (Expanded panel)',
                    'Prolactin (PRL)',
                    'Thyroid Stimulating Hormone (TSH)',
                    'Thyroxin, Free (FT4)',
                    'Triiodothyronine, Free (FT3)',
                    'Prostate Specific Antigen (PSA), Total',
                    'Prostate Specific Antigen (PSA), Free',
                    'Culture, Throat',
                    'Parasites, Stool',
                    'Hepatitis B s Antigen (HBsAg)',
                    'Hepatitis B s Antibody (anti-HBs)',
                    'Hepatitis C Antibody (anti-HCV)',
                    'HIV Antibody (1-2) + p24 Antigen',
                    'Insulin',
                ]
            ],
        ]
    ],
];
